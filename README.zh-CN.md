...
`s3disk` 当前是 pre-1.0 工程预览版。Linux 路径已经覆盖真实 S3 和 FUSE 端到端
测试；集成到产品前请先查看[平台支持](#平台支持)。
...
| macOS | 可以构建并运行原生测试；挂载需要用户另行安装 macFUSE，仍需针对具体产品做端到端验证。 |
