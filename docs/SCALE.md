...
eight concurrent readers. It emits one `S3DISK_SCALE_EVIDENCE=<json>` record and
GitHub Actions archives that record with the exact source revision and runner
environment. The defaults are deliberately small enough for every pull request;
they are a regression gate, not a production capacity claim.
